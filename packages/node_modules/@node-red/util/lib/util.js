...
function cloneMessage(msg) {
    if (typeof msg !== "undefined" && msg !== null) {
        ...
        delete msg.res;
        var m = {};
        console.log("[Node-RED Bun Fix] Starting to clone message");
        for (var key in msg) {
            if (msg.hasOwnProperty(key)) {
                var val = msg[key];
                if (val !== undefined && val !== null) {
                    if (typeof val === "function") {
                        // Funktionen können nicht mit structuredClone tiefenkopiert werden.
                        // Für gewöhnlich brauchen diese aber auch keinen Clone.
                        console.log("[Node-RED Bun Fix] Property '" + key + "' is a function -> kept as reference");
                        m[key] = val;
                    } else if (typeof val === "object") {
                        if (typeof structuredClone === "function") {
                            try {
                                m[key] = structuredClone(val);
                                console.log("[Node-RED Bun Fix] Property '" + key + "' is an object -> structuredClone succeeded");
                            } catch(e) {
                                console.log("[Node-RED Bun Fix] Property '" + key + "' is an object -> structuredClone failed (" + e.message + ") -> falling back to clonedeep");
                                m[key] = clonedeep(val);
                            }
                        } else {
                            console.log("[Node-RED Bun Fix] Property '" + key + "' is an object -> no structuredClone available -> using clonedeep");
                            m[key] = clonedeep(val);
                        }
                    } else {
                        console.log("[Node-RED Bun Fix] Property '" + key + "' is a primitive (" + typeof val + ") -> copied directly");
                        m[key] = val;
                    }
                } else {
                    console.log("[Node-RED Bun Fix] Property '" + key + "' is " + String(val) + " -> copied directly");
                    m[key] = val;
                }
            }
        }
        console.log("[Node-RED Bun Fix] Finished cloning message");
        if (req) {
            m.req = req;
            msg.req = req;
        }
