...
module.exports = function(RED) {
    ...
    var bunGateway = require("../../../../node-red/lib/bun-gateway");
    ...
    function createSocketAdapter(sendFn, closeFn) {
        var handlers = {};
        return {
            send: sendFn,
            close: closeFn,
    ...
    function createClientSocket(url, subprotocols) {
        var protocols = subprotocols && subprotocols.length ? subprotocols : undefined;
        var rawSocket = new WebSocket(url, protocols);
        var socket = createSocketAdapter(function(payload) {
            rawSocket.send(payload);
        }, function() {
            rawSocket.close();
        });

        rawSocket.addEventListener("open", function() {
            socket.readyState = WebSocket.OPEN;
            socket.emit("open");
        });
        rawSocket.addEventListener("message", function(event) {
            socket.emit("message", toMessageString(event.data), false);
        });
        rawSocket.addEventListener("close", function() {
            socket.readyState = WebSocket.CLOSED;
            socket.emit("close");
        });
        rawSocket.addEventListener("error", function(event) {
            socket.emit("error", event.error || new Error("WebSocket client error"));
        });

        socket.readyState = rawSocket.readyState;
        return socket;
    }

    function createServerSocket(socket) {
        var adapter = createSocketAdapter(function(payload) {
            socket.send(payload);
        }, function() {
            socket.close();
        });
    ...
    function WebSocketListenerNode(n) {
        ...
        function startconn() {    // Connect to remote endpoint
            ...
            if (!node._bunClientOptionsWarned && (options.agent || options.headers)) {
                node._bunClientOptionsWarned = true;
                node.warn(RED._("websocket.errors.send-error") + " Bun WebSocket client does not fully support ws-specific options (agent/custom headers)");
            }

            var socket = createClientSocket(node.path, node.subprotocol, options);
        ...
        if (node.isServer) {
            ...
            node._bunSocketMap = new WeakMap();
            bunGateway.registerWebSocket(node.fullPath, {
                open: function(socket) {
                    var adapter = createServerSocket(socket);
                    node._bunSocketMap.set(socket, adapter);
                    handleConnection(adapter);
                },
                message: function(socket, message) {
                    var adapter = node._bunSocketMap.get(socket);
                    if (adapter) {
                        node.handleEvent(adapter.nrId, adapter, 'message', toMessageString(message), false);
                    }
                },
                close: function(socket) {
                    ...
                    node._bunSocketMap.delete(socket);
                },
            ...
            node.server = {
                setMaxListeners: function() {},
                close: function() {
                    bunGateway.unregisterWebSocket(node.fullPath);
                }
            };
        ...
        node.on("close", function(done) {
            ...
            if (node.isServer) {
                ...
                let si = setInterval(() => {
                    if(node.server.readyState === WebSocket.CLOSED || closeMonitorCount <= 0) {
                        if (node.tout) {
                            clearTimeout(node.tout);
                            node.tout = null;
                        }
